...
SHT_status_t SHT1_read_temp_humidity(float* temp, float* humidity) {
    ...
    i2c_read_blocking_until(i2c0, 0x03, raw_temp, (size_t)STH1_BUF_LEN, false, SHT_i2c_timeout);

    // Read humidity
    uint8_t raw_humidity[STH1_BUF_LEN];
    i2c_read_blocking_until(i2c0, 0x03, raw_humidity, (size_t)STH1_BUF_LEN, false, SHT_i2c_timeout);
